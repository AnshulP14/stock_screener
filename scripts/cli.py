...
Usage:
    python cli.py                          # Both markets, incremental
    python cli.py --market nse --mode full # NSE500 full
    python cli.py --market us --mode full  # S&P 500 full
    python cli.py --market nse --mode quick # Top 50 NSE500
    python cli.py --market nse --symbols RELIANCE TCS
    python cli.py --dry-run                # Preview only
    python cli.py --workers 5              # More parallel workers
"""
...
from markets import nse as nse_market
from markets import us as us_market
...
def main() -> None:
    ...
    for label, run_fn in markets:
        ...
        if label == "NSE500" and mode == "rebuild":
            print(f"  NSE500: 'rebuild' not applicable, using 'transform-only'")
            mode = "transform-only"
